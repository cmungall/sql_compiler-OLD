...
'pack/$Name-$Version.tgz' <-- [],
  'pushd . && cd ~/releasedir && (test -d $Name && rm -rf $Name || echo making fresh) && git clone git@github.com:cmungall/$Name.git && rm -rf $Name/.git && tar zcvf $Name-$Version.tgz $Name && popd && mv ~/releasedir/$Name-$Version.tgz pack'.
